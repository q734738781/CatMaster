...
Research 会负责方向判断，也会继续把阶段任务交给可执行的 specialist。在用户授权的范围内，一条 Research thread 可以从文献证据缺口推进到结构建模、远程计算、结果复核、写作和独立审稿。具体领域操作由拥有相应 tools 与 skills 的 specialist 或 worker 完成；结果返回同一 workspace 后，Research 可以核对证据、补发下一阶段任务或收束结论。它的交付既包括研究路线，也包括各 specialist 实际完成的阶段产物。
...
Research decides how to advance an open objective and dispatches the stages to specialists that can execute them. Within the authority granted by the user, one Research thread can move from a literature evidence gap to structure modeling, remote computation, result checks, writing, and independent review. The specialist or worker that owns the relevant tools and skills performs each domain action. Research then reads the returned artifacts, checks the evidence, dispatches another stage, or closes the investigation. Its deliverables include both the research plan and the work completed by those specialists.
...
| Conor Bronsdon's [`avoid-ai-writing`](https://github.com/conorbronsdon/avoid-ai-writing) | Runtime writing-quality skill used for prose-heavy deliverables | MIT; local notice at [`skills/writing_quality/avoid-ai-writing/LICENSE`](skills/writing_quality/avoid-ai-writing/LICENSE). Its upstream documentation also credits [`brandonwise/humanizer`](https://github.com/brandonwise/humanizer) and the wider Humanizer skill ecosystem |
