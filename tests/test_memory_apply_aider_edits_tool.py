...
def test_memory_apply_aider_edits_success(tmp_path) -> None:
    with workspace_scope(tmp_path):
        ...
        content, artifact = memory_apply_aider_edits(
            {
                "edits_text": (
                    "MEMORY/MEMORY.md\n"
                    "<<<<<<< SEARCH\n"
    ...
    assert "memory_apply_aider_edits completed" in str(content).lower()
...
def test_memory_apply_aider_edits_rejects_invalid_blocks(tmp_path) -> None:
    with workspace_scope(tmp_path):
        with pytest.raises(CatMasterToolExecutionError):
            memory_apply_aider_edits({"edits_text": "not a valid aider edit"})
