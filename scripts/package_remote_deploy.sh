...
write_deploy_readme() {
  {
    ...
    printf '\n'
    printf '%s\n' 'The control-plane environment includes pinned ScanSci PDF and CloakBrowser integrations. Literature Review tries legal direct OA adapters first and keeps one internal ScanSci/CloakBrowser DOI-page fallback behind its high-level acquisition tool; no separate browser CLI or model-visible browser surface is required.'
    printf '\n'
