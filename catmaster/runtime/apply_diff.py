"""Apply OpenAI V4A update/create diffs to text.

Adapted from ``openai-agents-python/src/agents/apply_diff.py`` (Apache-2.0).
Keeping the parser local avoids adding the full Agents SDK to CatMaster's
control-plane environment solely for one Codex-compatible freeform tool.
"""
...
def _parse_update_diff(lines: list[str], input: str) -> ParsedUpdateDiff:
    ...
    while not _is_done(parser, END_SECTION_MARKERS):
        ...
        for chunk in section.section_chunks:
            chunks.append(
                Chunk(
                    orig_index=chunk.orig_index + find_result.new_index,
...
def _advance_cursor_to_anchor(
    ...
    if not any(line == anchor for line in input_lines[:cursor]):
        for index in range(cursor, len(input_lines)):
            if input_lines[index] == anchor:
                cursor = index + 1
                found = True
                break
    ...
    if not found and not any(line.strip() == anchor.strip() for line in input_lines[:cursor]):
        for index in range(cursor, len(input_lines)):
            if input_lines[index].strip() == anchor.strip():
                cursor = index + 1
                parser.fuzz += 1
                break
...
def _find_context_core(lines: list[str], context: list[str], start: int) -> ContextMatch:
    ...
    for index in range(start, len(lines)):
        if _equals_slice(lines, context, index, lambda value: value):
            return ContextMatch(new_index=index, fuzz=0)
    for index in range(start, len(lines)):
        if _equals_slice(lines, context, index, lambda value: value.rstrip()):
            return ContextMatch(new_index=index, fuzz=1)
    for index in range(start, len(lines)):
        if _equals_slice(lines, context, index, lambda value: value.strip()):
            return ContextMatch(new_index=index, fuzz=100)
