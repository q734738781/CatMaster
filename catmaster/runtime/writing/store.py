...
class WritingStore:
    def __init__(self, *, workspace: Path, run_id: str) -> None:
        ...
        self.metadata_root = system_root(self.workspace) / "writing_runs" / self.run_id
        self.files_root = workspace_root(self.workspace) / "writing" / self.run_id

    @property
    def sections_dir(self) -> Path:
        return self.metadata_root / "sections"

    @property
    def reviews_dir(self) -> Path:
        return self.metadata_root / "reviews"

    @property
    def latex_dir(self) -> Path:
        return self.files_root / "latex"
    ...
    def write_request(self, payload: dict[str, Any]) -> str:
        self.ensure_exists()
        path = self.metadata_root / "writing_request.json"
    ...
    def persist_plan(self, plan: WritingPlanModel) -> str:
        self.ensure_exists()
        path = self.metadata_root / "writing_plan.json"
    ...
    def persist_bundle(self, bundle: ManuscriptBundleModel) -> str:
        self.ensure_exists()
        path = self.metadata_root / "manuscript_bundle.json"
