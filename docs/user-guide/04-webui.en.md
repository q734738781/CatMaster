...
An unbound thread's first Research turn is attached before the model starts. If the workspace has one open, non-archived graph, CatMaster reuses it; if it has none, CatMaster creates a manual graph whose question is the current Research request. If several open graphs exist, the turn stays unbound so you can choose explicitly. This default applies only to the Research entry: a direct Experiment, Literature Review, or Writing turn does not silently choose or create a graph. Automatic attachment supplies navigation but does not force a one-off request to create scientific nodes.
...
Creating a graph through the Research Specialist also attaches it to the current
