...
### Unified MLFF tasks: SP, relax, MD, NEB, and TS

`mlff_sp`, `mlff_relax`, `mlff_md`, `mlff_neb`, `mlff_vib`, and `mlff_ts` use common task names. The backend configuration selects MACE, FairChem UMA, MatterSim, or ORB-v3. This keeps one scientific workflow across enabled providers rather than duplicating tools for every model family.
...
`mlff_sp`, `mlff_relax`, and `mlff_vib` can process several structures directly under one stage's `input/`, so a multi-structure screen is not automatically a remote batch. `mlff_md` accepts one start or restart structure. `mlff_neb` accepts a locally constructed, validated fixed-image path. `mlff_ts` accepts exactly one TS-like structure directly under `input/`.

Constrained MLFF optimization inherits atom constraints from the structure file rather than a separate `fixed_atoms` override. POSCAR/VASP uses Selective Dynamics. Extxyz uses the standard `move_mask` property (`L:1` for whole atoms or `L:3` for Cartesian components, with false meaning fixed). `mlff_sp`, `mlff_relax`, and `mlff_ts` retain extxyz input as `sp.extxyz`, `opt.extxyz`, or `ts.extxyz` and verify the constraint mask after writing. Extxyz cannot represent ASE scaled-coordinate `FixScaled` constraints; use POSCAR/VASP for that case. Keep `relax_cell=false` for the usual fixed-atom surface relaxation because constrained atoms may otherwise move affinely with a changing cell.

`mlff_ts` uses fixed-cell, order-one constrained RS-pRFO. Its `auto` Hessian strategy uses a calculator's public analytic Hessian when available, a constraint-projected finite-difference Hessian for small systems, and iterative diagonalization for larger systems. Final frequency validation is independent of optimization convergence. A completed optimization is marked `validated_first_order_saddle=true` only when it converged and exactly one mode is more negative than the configured imaginary-frequency threshold.

`mlff_vib` is general normal-mode analysis rather than a TS-only task. It can analyze minima, transition states, molecules, adsorbates, and constrained material structures. It uses the same exact constraint projection and final mass-weighted eigensolver as `mlff_ts`, but never optimizes the geometry. Complete spectra use an analytic Hessian when the calculator exposes one or finite differences in the free-coordinate subspace otherwise. Results are compact: `vibrations.npz` contains the geometry, masses, constraint basis, reduced Hessian, frequencies, and mass-normalized modes; `frequencies.csv` is the table and `modes.extxyz` is one multi-frame viewer file. No ASE per-displacement JSON cache is retained. For a periodic supercell this is a Gamma-point finite-supercell analysis, not a phonon dispersion.
