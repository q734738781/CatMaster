...
- A Result contains a concise outcome. Typed relationships state which hypotheses it supports, opposes, or does not distinguish.
...
Users can create seed hypotheses, experiment proposals, and results themselves. They can also start a bound Research planning thread from a Result. The `hypothesis_proposer` and `evidence_judge` still provide independent scientific planning and evidence interpretation. Automatic mode starts one ready Experiment at a time. If none is ready, it starts one planning child. A planner that makes no graph change at the current revision is not restarted in a loop.
...
Research can call `list_research_graphs`, `create_research_graph`, `inspect_research_graph`, `add_research_hypothesis`, `add_research_experiment`, `record_research_result`, and `mark_research_experiment_failed`. These tools require an explicit graph ID and current revision and do not accept generic metadata. Persistent state lives in the workspace `metadata/workspace.sqlite`. The graph stores scientific nodes, relationships, and refs. Detailed execution records and resource use remain in the existing thread, receipt, and artifact stores.
