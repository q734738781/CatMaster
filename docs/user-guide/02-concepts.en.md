...
Delegation carries the scientific contract, not a low-level runbook. Research passes the objective, evidence, scientific conditions or comparison rules that must remain unchanged, compute authority, and stopping point. Experiment may add the responsible worker and canonical input or output paths. The worker then owns preparation, compatible execution routing, implementation-level corrections, recovery, and domain QC within those boundaries. Preparation, smoke testing, submission, and recovery should not be split into separate delegations merely because they are separate mechanical steps.

A failed specialist choice is not automatically a user blocker. If a named worker, task keyword, backend, or prescribed step is unsuitable, the worker returns the concrete mismatch to its delegator. Experiment first rewrites the brief or selects a different compatible worker or route while preserving the same scientific model, system, conditions, comparison rule, and intended evidence. If that equivalent route lies outside Experiment's ownership, a Research-delegated Experiment returns it to Research for rerouting. Human input is required only when no authorized scientifically equivalent route remains, or when continuing would change an explicit user requirement, user-controlled scientific choice, approved cost or time, or a safety or authorization boundary.
