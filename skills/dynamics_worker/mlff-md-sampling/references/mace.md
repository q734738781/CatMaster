...
MACE backend controls include model/checkpoint, head, dispersion, `default_dtype`, `enable_cueq`, `compile_mode`, and device. A staged checkpoint replaces the model alias. Put it under the stage's `models/` directory and pass its stage-relative path; the submission validator records its SHA-256 and size.
