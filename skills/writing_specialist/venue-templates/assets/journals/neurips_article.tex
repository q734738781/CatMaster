...
\textbf{Implementation Details:} Provide result-bearing hyperparameters and implementation conditions. Put any venue-required resource disclosure in the checklist or appendix; omit incidental hardware and launcher details from the scientific narrative.
...
Use only claim-critical ablations that establish which mechanism produces the target advantage.

\subsection{Analysis}
Explain why the target advantage occurs, the conditions under which the evidence supports it, and the most plausible alternative explanation ruled out by the experiments.

\section{Discussion}
\label{sec:discussion}

Interpret the main finding and its scientific or practical implications. Keep the discussion centered on the strongest supported contribution.

\subsection{Scope Disclosure (only if required)}
If the venue requires a dedicated scope or limitations disclosure, state only the precise condition that materially changes the central claim. Do not add a generic weakness or future-work inventory.
