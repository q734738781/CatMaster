...
- MACE MD reads the last input frame and preserves its momenta when present; use the prior `restart.traj` for segmented sampling so the true final frame and compatible RNG state are restored. Existing velocities are replaced only when `dynamics.reinitialize_velocities=true` is explicitly requested.
